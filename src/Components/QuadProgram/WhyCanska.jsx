...
const customTabsTheme = {
  tablist: {
    // Container with frosted-glass overlay style from your Figma specs
    base: "flex flex-wrap items-center gap-2 p-4 bg-[rgba(251,250,255,0.9)] border-y border-[#EDE7FB] backdrop-blur-[12px] overflow-x-auto",
    styles: {
      pills: {
        base: "",
        active: {
          // ACTIVE TAB: Purple pill with drop-shadow
          on: " bg-[#5E17EB] text-white shadow-[0px_10px_15px_-3px_rgba(94,23,235,0.25),0px_4px_6px_-4px_rgba(94,23,235,0.25)]",
          // INACTIVE TAB: White pill with border
          off: " bg-white text-[rgba(10,10,10,0.75)] border border-[#EDE7FB] hover:bg-gray-50",
        },
      },
    },
    tabitem: {
      base: "flex items-center gap-2 px-5 py-2.5 rounded-full font-['DM_Sans'] text-sm font-semibold transition-all duration-200 focus:outline-none whitespace-nowrap",
    },
  },
};
  ...
  const renderTabTitle = (number, text, index) => {
    ...
    return (
      <div className="flex items-center gap-2.5">
        <span
          className={`flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold transition-colors duration-200 ${isActive
